...
def run_npop(data, threshd, dir_out):
    ...
    io.output_data(data, indices, dir_out, option="MOpop", data2=corrmat)
...
def run_efield(data, threshd, dir_out):
    ...
    io.output_data(np.stack((data_w, data_s)), indices, dir_out, option="efield")
...
def run_nstate(data, threshd, dir_out):
    ...
    indices = []
    io.output_data(
        np.stack((time, aucofu)),
        indices,
        dir_out,
        option="aucofu",
        data2=np.stack((data_w, data_s)),
    )
...
def main(data_in, dir_in, dir_out, threshd=1.0e-5):
