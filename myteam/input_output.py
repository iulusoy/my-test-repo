...
class output_data:
    ...
    def plot0(self):
        ...
        for i in range(1, len(self.data.columns)):
            ...
            plt.savefig(
                "{}/Figure_expec_{}.pdf".format(self.outdir, self.index[i]),
                dpi=300,
                bbox_inches="tight",
            )
    ...
    def plot1(self):
        ...
        plt.savefig(
            "{}/Figure_npopcorr.pdf".format(self.outdir), dpi=300, bbox_inches="tight"
        )
        print("Writing correlation matrix")
        self.data2.to_csv("{}/corrmat.dat".format(self.outdir), header=True, sep=" ")
    ...
    def plot2(self):
        ...
        plt.savefig(
            "{}/Figure_l2norm.pdf".format(self.outdir), dpi=300, bbox_inches="tight"
        )
    ...
    def plot3(self):
        ...
        for i in range(1, len(self.data)):
            ...
            plt.savefig(
                "{}/Figure_efield_FT_{}.pdf".format(
                    self.outdir, labels[self.index[0][i]]
                ),
                dpi=300,
                bbox_inches="tight",
            )
    ...
    def plot4(self):
        ...
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.plot(np.real(self.data[0]), np.real(self.data[1]), label="real part")
        ax.plot(np.real(self.data[0]), np.imag(self.data[1]), label="imaginary part")
        ax.plot(np.real(self.data[0]), np.abs(self.data[1]), label="absolute value")
        self.plotparams(ax, "time (fs)", "autocorrelation function")
        plt.savefig(
            "{}/Figure_autocorrelation_function.pdf".format(self.outdir),
            dpi=300,
            bbox_inches="tight",
        )
        print("Plotting FT of autocorrelation function")
        fig, ax = plt.subplots(figsize=(8, 5))
        ax.plot(np.real(self.data2[0]), np.real(self.data2[1]), label="real part")
        ax.plot(np.real(self.data2[0]), np.imag(self.data2[1]), label="imaginary part")
        ax.plot(np.real(self.data2[0]), np.abs(self.data2[1]), label="absolute value")
        self.plotparams(ax, "energy (au)", "Fourier transform")
        plt.savefig(
            "{}/Figure_autocorrelation_function_FT.pdf".format(self.outdir),
            dpi=300,
            bbox_inches="tight",
        )
